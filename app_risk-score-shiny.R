...
# ══════════════════════════════════════════════════════════════════
ui <- fluidPage(
  title = "AprendiZAP · Score de Risco",
  ...
  div(class="header-bar",
    h3("AprendiZAP — Score de Risco de Abandono"),
    p("PADS 2026 · Fundação 1Bi · Ilana García · Izabelle Silva · Julia Navarro · Lívia Bertoni")
  ),
    ...
    tabPanel("Robustez", br(),
      fluidRow(
        column(4, div(class="sidebar-panel",
          ...
          sliderInput("w5","Sem Mari IA (base: 1)",   min=0, max=3, value=1),
      ...
      fluidRow(column(8, offset=2,
        div(style="background:white;border-radius:6px;padding:32px;box-shadow:0 1px 3px rgba(0,0,0,.06);",
          ...
          tags$table(style="width:100%;border-collapse:collapse;",
            ...
            tags$tr(tags$td(style="padding:8px;","Sem uso da Mari IA"),
                    tags$td(style="padding:8px;text-align:center;font-weight:bold;","1"),
...
server <- function(input, output, session) {
  ...
  output$plot_sensibilidade <- renderPlot({
    ...
    labels_peso <- c("sessao unica","sem 2o acesso","dia unico","sem conteudo","sem Mari IA","sem formacao")
